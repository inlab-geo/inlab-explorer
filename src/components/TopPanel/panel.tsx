...
const Panel: React.FC<onClick> = ({ onClickTree, onClickTheme }) => {
    ...
    return (
      ...
      >
        ...
        {badges.map((badge, index) => (
          <React.Fragment key={index}>{singleBadge(badge)}</React.Fragment>
        ))}
      </div>
    );
  }
    ...
    return (
      <div>
        ...
        >
          ...
          <ToggleButton
            value="Espresso Problems"
            aria-label="Espresso Problems"
          >
            {styleText("Espresso Problems")}
          </ToggleButton>
        </ToggleButtonGroup>
  ...
  //-------------------------------------------------------------
  return <div style={{ height: `5vh`, width: "100%" }}>{bar()}</div>;
};
