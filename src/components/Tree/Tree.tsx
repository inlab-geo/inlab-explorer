...
const TreeComponent: React.FC<selected> = ({
  selectedTree,
  selectedTheme,
  popupcont,
  setPopup,
  ...
  useEffect(() => {
    ...
    if (d3Container.current) {
      ...
      svg.selectAll(".node").style("fill", function (d: any) {
  ...
  useEffect(() => {
    switch (selectedTheme) {
      case "use light theme": {
        setTreeTheme(style.LightTheme);
        break;
      }
      case "use dark theme": {
        setTreeTheme(style.DarkTheme);
        break;
      }
      case "use special theme": {
        setTreeTheme(style.SpecialTheme);
        break;
      }
    }
  }, [selectedTheme]);
  ...
  function gen_button(
    ...
    function getLinkDoc(d: TreeProps) {
      return d.data?.link_doc
        ? `<a href=" ${d.data
            ?.link_doc} " target="_blank" style="text-decoration: none;"><button style="background-color: ${
            d.data?.link_doc ? color : "#b8b8b8"
          }; width: ${width}px; border: none; color: white; padding: 5px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px; margin: 4px 2px; cursor: pointer; border-radius: 12px; box-sizing: border-box;">Documentation</button></a>`
        : " ";
    }
    function getLinkGit(d: TreeProps) {
      return d.data?.link_git
        ? `<a href=" ${d.data
            ?.link_git} " target="_blank" style="text-decoration: none;"><button style="background-color: ${
            d.data?.link_git ? color : "#b8b8b8"
          }; width: ${width}px; border: none; color: white; padding: 5px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px; margin: 4px 2px; cursor: pointer; border-radius: 12px; box-sizing: border-box;">Link to GitHub</button></a>`
        : " ";
    }
  ...
  useEffect(() => {
    ...
    if (treeData && d3Container.current) {
      ...
      const update = (source: any) => {
        ...
        var node = g
          .selectAll<SVGGElement, HierarchyPointNode<unknown>>("g.node")
              ...
              if (d.children || d._children) {
                if (
                  ...
                  (d._children &&
                    ...
                    (d._children[0].children || d._children[0]._children))
                ) {
        ...
        var link = g
          .selectAll<SVGPathElement, HierarchyPointNode<unknown>>("path.link")
        ...
        function click(event: any, d: any) {
          ...
          svg
            ...
            .call(
              ...
              d3.zoomIdentity
                .translate(
                  (center.w - d.x) * currentScale +
                    (1.05 - currentScale) * center.w,
                  (center.h - d.y) * currentScale +
                    (1.05 - currentScale) * center.h,
                )
                .scale(currentScale),
            );
        }
